...
{
    public partial class GovernmentApproval : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                strQuickLink = Functions.CreateQuickLink("Home", "GovernmentApproval");
            }
        }
        private string GetHeaderImage()
        {
            ...
            {
                var dataMain = objBlogCategoryMasterRepository.GetBreadCumImageByPageName(languageId, "GovernmentApproval").FirstOrDefault();
                ...
                LableData:
                    ...
                    {
                        languageId = 1;
                        dataMain = objBlogCategoryMasterRepository.GetBreadCumImageByPageName(languageId, "GovernmentApproval").FirstOrDefault();
                        if (languageId != 1)
                        {
                            goto LableData;
                        }
        ...
        private string GetPageData()
        {
            ...
            {
                DataSet ds = new GovApprovelBAL().SelectGov_Approvel(languageId);
                LableData:
                if (ds != null)
                {
                    strBoardOfDirector = new StringBuilder();
                    if (!ds.Tables.Count.Equals(0) && !ds.Tables[0].Rows.Count.Equals(0))
                    {
                        ...
                        strBoardOfDirector.Append("						<th>Year Of Approvel</th>");
                        strBoardOfDirector.Append("						<th>Latter of Permission</ th>");
                        strBoardOfDirector.Append("					</tr>");
                        strBoardOfDirector.Append("				</thead>");
                        ...
                        foreach (DataRow row in ds.Tables[0].Rows)
                        {
                            ...
                            if (!string.IsNullOrWhiteSpace(row["Doc_path"].ToString()))
                            {
                                strURL = (row["Doc_path"].ToString());
                            ...
                            ResolveUrl(("~/DoctorProfile?" + Functions.Base64Encode(row["Doc_path"].ToString())));
                            strBoardOfDirector.Append("");
                            strBoardOfDirector.Append("<tr>");
                            strBoardOfDirector.Append("	<td>" + i + "</td>");
                            strBoardOfDirector.Append("	<td>" + row["CourseName"] + "</td>");
                            strBoardOfDirector.Append("	<td>" + row["Year_id"] + "</td>");
                    ...
                    else
                    {
                        ...
                        {
                            languageId = 1;
                        ds = new GovApprovelBAL().SelectGov_Approvel(languageId);
                            goto LableData;
                        }
                    }
                }
