...
{
    public partial class CallUsTypeMaster : System.Web.UI.Page
    {
        #region Page Load
        ...
        protected void gView_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                if ((e.CommandName.Equals("eView")) || (e.CommandName.Equals("eEdit")) || (e.CommandName.Equals("eDelete")))
                {
                    ...
                    {
                        CallUsTypeMasterBO objBo = new CallUsTypeMasterBO();
                        objBo.CallUs_id = bytID;
                        new CallUsTypeMasterBAL().DeleteRecord(objBo);
                        BindGridView();
        ...
        private bool FillControls(Int32 iPkId, int languageId)
        {
            ...
            DataSet ds = new CallUsTypeMasterBAL().SelectRecord(objBo);
            if (ds.Tables.Count.Equals(0) || ds.Tables[0].Rows.Count.Equals(0))
            {
                txtCallUsTypeName.Text = "";
            }
            else
            {
                DataRow dr = ds.Tables[0].Rows[0];
                if (dr.HasErrors) return false;
                if (dr["CallUs_Name"] != DBNull.Value)
                    txtCallUsTypeName.Text = Convert.ToString(dr["CallUs_Name"]);
        ...
        private void LoadControls(CallUsTypeMasterBO objBo)
        {
            objBo.CallUs_Name = txtCallUsTypeName.Text;
        ...
        protected void btn_Save_Click(object sender, EventArgs e)
        {
            try
            {
                CallUsTypeMasterBO objBo = new CallUsTypeMasterBO();
                LoadControls(objBo);
                if (new CallUsTypeMasterBAL().InsertRecord(objBo))
                {
        ...
        protected void btn_Update_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (new CallUsTypeMasterBAL().UpdateRecord(objBo))
                {
