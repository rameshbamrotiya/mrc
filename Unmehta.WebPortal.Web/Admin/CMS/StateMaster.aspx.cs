...
namespace Unmehta.WebPortal.Web.Admin.CMS
{
    public partial class StateMaster : System.Web.UI.Page
    {
        ...
        protected void gView_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                if ((e.CommandName.Equals("eView")) || (e.CommandName.Equals("eEdit")) || (e.CommandName.Equals("eDelete")))
                {
                    ...
                    Int32 bytID;
                    bytID = Convert.ToInt32(gView.DataKeys[intIndex].Values["State_id"]);
                    if (e.CommandName == "eDelete")
                    {
                        StateBO objBo = new StateBO();
                        objBo.State_id = bytID;
                        new StateBAL().DeleteRecord(objBo);
                        BindGridView();
        ...
        {
            StateBO objBo = new StateBO();
            objBo.State_id = iPkId;
            objBo.LanguageId = languageId;
            hfTemplateId.Value = iPkId.ToString();
            DataSet ds = new StateBAL().SelectRecord(objBo);
            ...
            else
            {
                ...
                if (dr["State_Name"] != DBNull.Value)
                    txtStateName.Text = Convert.ToString(dr["State_Name"]);
                FillCountry();
        ...
        private void LoadControls(StateBO objBo)
        {
            objBo.State_Name = txtStateName.Text;
        ...
        protected void btn_Save_Click(object sender, EventArgs e)
        {
            try
            {
                StateBO objBo = new StateBO();
                LoadControls(objBo);
                if (new StateBAL().InsertRecord(objBo))
                {
        ...
        protected void btn_Update_Click(object sender, EventArgs e)
        {
            try
            {
                StateBO objBo = new StateBO();
                LoadControls(objBo);
                objBo.State_id = Convert.ToInt32(ViewState["PK"]);
                if (new StateBAL().UpdateRecord(objBo))
                {
        ...
        private void FillCountry()
        {
            ...
            PopulateDropDownList(ddlCountry, dt, "Country_Name", "Country_id", true);
        }
    }
}