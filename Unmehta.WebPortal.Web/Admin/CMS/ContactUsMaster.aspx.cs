...
namespace Unmehta.WebPortal.Web.Admin.CMS
{
    public partial class ContactUsMaster : System.Web.UI.Page
    {
        ...
        private void LoadControls(ContactUsMasterBO objBo)
        {
            objBo.Language_id = Convert.ToInt16(drpLanguage.SelectedValue.ToString());
            objBo.ContactUsDescription = HttpUtility.HtmlEncode(txtContactUsDescription.Text.ToString());
        ...
        protected void btn_Save_Click(object sender, EventArgs e)
        {
            try
            {
                ContactUsMasterBO objBo = new ContactUsMasterBO();
                LoadControls(objBo);
                if (new ContactUsMasterBAL().InsertRecord(objBo))
                {
        ...
        {
            ContactUsMasterBAL objbal = new ContactUsMasterBAL();
            DataSet ds = objbal.SelectContactUsDetails();
            DataTable dt = ds.Tables[0];
            if (dt.Rows.Count > 0)
            {
                btn_Add.Visible = false;
                dt.Rows[0]["ContactUsDescription"] = HttpUtility.HtmlDecode(dt.Rows[0]["ContactUsDescription"].ToString());
                dt.AcceptChanges();
                gView.DataSource = dt;
                gView.DataBind();
            }
        ...
        protected void btn_Update_ServerClick(object sender, EventArgs e)
        {
            try
            {
                ContactUsMasterBO objBo = new ContactUsMasterBO();
                LoadControls(objBo);
                objBo.ContactUs_Id = Convert.ToInt16(hdPKId.Value.ToString());
                if (new ContactUsMasterBAL().UpdateRecord(objBo))
                {
        ...
        private void FillControls(int PKId, int Languageid)
        {
            try
            {
                ...
                if (dtDetails.Rows.Count > 0)
                {
                    ...
                    txtContactUsDescription.Text = HttpUtility.HtmlDecode(dr["ContactUsDescription"].ToString());
                }
                else
                {
                    ClearControlValues(pnlEntry);
        ...
        protected void ibtn_Delete_Click(object sender, EventArgs e)
        {
            ...
            {
                int rowindex = ((GridViewRow)(sender as LinkButton).NamingContainer).RowIndex;
                int rowId = Convert.ToInt32(gView.DataKeys[rowindex]["ContactUs_Id"].ToString());
                ContactUsMasterBO objBo = new ContactUsMasterBO();
                objBo.ContactUs_Id = rowId;
                new ContactUsMasterBAL().DeleteRecord(objBo);
