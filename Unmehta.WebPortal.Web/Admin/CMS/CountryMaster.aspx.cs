...
namespace Unmehta.WebPortal.Web.Admin.CMS
{
    public partial class CountryMaster : System.Web.UI.Page
    {
        ...
        protected void gView_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                if ((e.CommandName.Equals("eView")) || (e.CommandName.Equals("eEdit")) || (e.CommandName.Equals("eDelete")))
                {
                    ...
                    Int32 bytID;
                    bytID = Convert.ToInt32(gView.DataKeys[intIndex].Values["Country_id"]);
                    if (e.CommandName == "eDelete")
                    {
                        CountryBO objBo = new CountryBO();
                        objBo.Country_id = bytID;
                        new CountryBAL().DeleteRecord(objBo);
                        BindGridView();
        ...
        {
            CountryBO objBo = new CountryBO();
            objBo.Country_id = iPkId;
            objBo.LanguageId = languageId;
            hfTemplateId.Value = iPkId.ToString();
            DataSet ds = new CountryBAL().SelectRecord(objBo);
            ...
            else
            {
                ...
                if (dr["Country_Name"] != DBNull.Value)
                    txtCountryName.Text = Convert.ToString(dr["Country_Name"]);
        ...
        private void LoadControls(CountryBO objBo)
        {
            objBo.Country_Name = txtCountryName.Text;
        ...
        protected void btn_Save_Click(object sender, EventArgs e)
        {
            try
            {
                CountryBO objBo = new CountryBO();
                LoadControls(objBo);
                if (new CountryBAL().InsertRecord(objBo))
                {
        ...
        protected void btn_Update_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                CountryBO objBo = new CountryBO();
                LoadControls(objBo);
                objBo.Country_id = Convert.ToInt32(ViewState["PK"]);
                if (new CountryBAL().UpdateRecord(objBo))
                {
