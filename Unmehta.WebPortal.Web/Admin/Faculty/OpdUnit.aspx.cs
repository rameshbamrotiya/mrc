...
{
    public partial class OpdUnit : System.Web.UI.Page
    {
        #region Page Method
        ...
        private bool LoadControlsAdd(OpdUnitGridModels objBo)
        {
            ...
            if (!string.IsNullOrEmpty(txtUnitName.Text))
                objBo.UnitName = txtUnitName.Text;
        ...
        {
            string errorMessage = "";
            using (IOpdUnitRepository objOpdUnitRepository = new OpdUnitRepository(Functions.strSqlConnectionString))
            {
                OpdUnitGridModels objBO = new OpdUnitGridModels();
                if (LoadControlsAdd(objBO))
                {
                    if (!objOpdUnitRepository.InsertOrUpdateTblOpdUnit(objBO, out errorMessage))
                    {
        ...
        protected void ibtn_Edit_Click(object sender, EventArgs e)
        {
            ...
            txtUnitName.Text = grdUser.Rows[rowindex].Cells[1].Text.Trim();
        ...
        protected void ibtn_Delete_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (IOpdUnitRepository objOpdUnitRepository = new OpdUnitRepository(Functions.strSqlConnectionString))
                {
                    objOpdUnitRepository.RemoveTblOpdUnit(rowId, out errorMessage);
                    ClearControlValues();
                    BindGridView();
                    Functions.MessagePopup(this, errorMessage, PopupMessageType.success);
                }
