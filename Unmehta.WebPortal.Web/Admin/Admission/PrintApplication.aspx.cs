    ...
    public partial class PrintApplication : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    Response.Redirect("~/Admin/Admission/ApplicationList");
                }
                string strQueryString = Functions.Base64Decode(HttpUtility.UrlDecode(strEndQueryString));

                if (strQueryString.Contains("AdminPAgeSec"))
                {
                ...
                if (!string.IsNullOrWhiteSpace(dtBasicDetail.Rows[0]["RegistrationId"].ToString()))
                {
                    lblApplicationNo.InnerHtml = dtBasicDetail.Rows[0]["RegistrationId"].ToString();
                }
                if (!string.IsNullOrWhiteSpace(dtBasicDetail.Rows[0]["CourseName"].ToString()))
                {
                    Label1.InnerHtml = dtBasicDetail.Rows[0]["CourseName"].ToString();
                    lblpostre.InnerHtml = dtBasicDetail.Rows[0]["CourseName"].ToString();
                }
                string lastName = "";
                ...
                if (!string.IsNullOrWhiteSpace(dtBasicDetail.Rows[0]["TxnId"].ToString()))
                {
                    finalPaymentTranction.InnerHtml = dtBasicDetail.Rows[0]["TxnId"].ToString() ;
                }
                if (!string.IsNullOrWhiteSpace(dtBasicDetail.Rows[0]["amount"].ToString()))
                {
                    finalPaymentAmount.InnerHtml = dtBasicDetail.Rows[0]["amount"].ToString();
                }
                if (!string.IsNullOrWhiteSpace(dtBasicDetail.Rows[0]["TransactionDate"].ToString()))
                {
                    finalPaymentDate.InnerHtml = dtBasicDetail.Rows[0]["TransactionDate"].ToString();
                }
                ...
                {
                    lblgen.InnerHtml = dtBasicDetail.Rows[0]["Gender"].ToString();
                ...
                if (!string.IsNullOrEmpty(dtBasicDetail.Rows[0]["CastName"].ToString()))
                {
                    LblCast.InnerHtml = dtBasicDetail.Rows[0]["CastName"].ToString();
                }                
                ...
                if (dtEducationDetail.Rows.Count > 0)
                {
                    ...
                    "</td>" +
                    "<td class='style12'>" +
                    dtEducationDetail.Rows[a]["NameOfSchoolCollege"].ToString() +
                    "</td>" +
                    "<td class='style33'>" +
                    ...
                    "</td>" +
                    "<td>" +
                    dtEducationDetail.Rows[a]["Stream"].ToString() +
                    "</td>" +
                    "<td class='style11'>" +
                    dtEducationDetail.Rows[a]["PercentageOrPercentile"].ToString() +
                    "</td>" +
                    "<td class='style11'>" +
                    dtEducationDetail.Rows[a]["NoOfTrials"].ToString() +
                    "</td>" +
                    "</tr>";
                    }
                    tes = tes + "</table>";
        ...
        private DataSet GetData(long candId)
        {
            ...
            command.CommandText = "GetAllAdmissionDetailForPrintForm";
            command.Parameters.AddWithValue("@StudentId", Convert.ToInt64(strStudentId));
            command.Parameters.AddWithValue("@CourseId", Convert.ToInt64(strCourseId));
            ...
            using (MemoryStream memoryStream = new MemoryStream())
            {
                ...
                if (ds != null && ds.Tables.Count > 0)
                {
                    ...
                    string strBody = "Dear " + LblFnameRe.InnerHtml +
                                       ...
                                       "<br/><br/> Course Name : " + lblpostre.InnerHtml +
                    ...
                    if (Functions.SendEmail(emailId, "Application Confirmation For The Course Of " + lblpostre.InnerHtml, strBody,out strError, true, lstAttachment))
                    {
        ...
        protected void btnBack_Click(object sender, EventArgs e)
        {
            ...
            //Response.Redirect("~/Admin/Admission/ApplicationList?" + strEndQueryString1);
            Response.Redirect("~/Admission/Course");
        }
    }
}