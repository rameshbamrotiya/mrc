...
function GetEditData(Data) {
    ...
    document.getElementById("bodyPart_txtReligionName").value = strData[1];
    document.getElementById("bodyPart_chkEnable").checked = strData[2].toLowerCase() == "true" ? 1 : 0;
}

function RemoveData(id) {
    if (confirm("Are you sure want to delete ? ")) {
        var urlS = "/Admin/Recruitment/Religion.aspx/RemoveDetailById";
        $.ajax({
            type: "POST",
            url: urlS,
            data: "{'id':'" + id + "'}",
...
function GetGriDData() {
    $.ajax({
        type: "POST",
        url: "/Admin/Recruitment/Religion.aspx/GetGridView",
        //data: "{'id':'1'}",
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        success: function (data) {
