...
function GetEditData(Data) {
    ...
    document.getElementById("bodyPart_txtEquipmentName").value = strData[2];
    document.getElementById("bodyPart_chkEnable").checked = strData[3].toLowerCase() == "true" ? 1 : 0;
    document.getElementById("bodyPart_hfFilName").value = strData[4];
}

function RemoveData(id) {
    if (confirm("Are you sure want to delete ? ")) {
        var urlS = "/Admin/Hospital/Equipment.aspx/RemoveDetailById";
        $.ajax({
            type: "POST",
            url: urlS,
            data: "{'id':'" + id + "'}",
...
function GetGriDData() {
    $.ajax({
        type: "POST",
        url: "/Admin/Hospital/Equipment.aspx/GetGridView",
        //data: "{'id':'1'}",
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        success: function (data) {
