...
{
    public partial class WetrateMaster : System.Web.UI.Page
    {
        #region Page Load
        ...
        protected void gView_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                if ((e.CommandName.Equals("eView")) || (e.CommandName.Equals("eEdit")) || (e.CommandName.Equals("eDelete")))
                {
                    ...
                    Int32 bytID;
                    bytID = Convert.ToInt32(gView.DataKeys[intIndex].Values["W_id"]);
                    if (e.CommandName == "eDelete")
                    {
                        WetreatBO objBo = new WetreatBO();
                        objBo.W_id = bytID;
                        new WetreatBAL().DeleteRecord(objBo);
                        BindGridView();
        ...
        private bool FillControls(Int32 iPkId, int languageId)
        {
            ...
            DataSet ds = new WetreatBAL().SelectRecord(objBo);
            if (ds.Tables.Count.Equals(0) || ds.Tables[0].Rows.Count.Equals(0)) return false;
            DataRow dr = ds.Tables[0].Rows[0];
            if (dr.HasErrors) return false;
            if (dr["WetreatDesc"] != DBNull.Value)
                txtWdesc.Text = Convert.ToString(dr["WetreatDesc"]);
            if (dr["Description"] != DBNull.Value)
                txtWetreatpopupdesc.Text = Convert.ToString(dr["Description"]);
            if (dr["is_active"] != DBNull.Value)
                ddlActiveInactive.SelectedValue = Convert.ToString(dr["is_active"]);
            if (dr["Readmore"] != DBNull.Value)
                ddlreadmore.SelectedValue = Convert.ToString(dr["Readmore"]);
        ...
        private void LoadControls(WetreatBO objBo)
        {
            ...
            objBo.WetreatDesc = txtWdesc.Text;
            objBo.DepartmentId = Convert.ToInt32(hfId.Value);
            objBo.Description = txtWetreatpopupdesc.Text;
            objBo.Readmore = ddlreadmore.SelectedValue.ToString();
        ...
        protected void btn_Save_Click(object sender, EventArgs e)
        {
            try
            {
                if (ddlreadmore.SelectedValue == "True")
                {
                    if (txtWetreatpopupdesc.Text == "")
                    {
                        Functions.MessagePopup(this, "Please enter Wetreat popup Description.", PopupMessageType.warning);
                        return;
                    }
                }
                WetreatBO objBo = new WetreatBO();
                LoadControls(objBo);
                if (new WetreatBAL().InsertRecord(objBo))
                {
        ...
        protected void btn_Update_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                WetreatBO objBo = new WetreatBO();
                LoadControls(objBo);
                objBo.W_id = Convert.ToInt32(ViewState["PK"]);
                if (new WetreatBAL().UpdateRecord(objBo))
                {
        ...
        protected void ddlreadmore_SelectedIndexChanged(object sender, EventArgs e)
        {
            redamore();
        }
        protected void redamore()
        {
            if (ddlreadmore.SelectedValue == "True")
            {
                txtWetreatpopupdesc.Visible = true;
            }
            else
            {
                txtWetreatpopupdesc.Visible = false;
            }
        }
    }
}